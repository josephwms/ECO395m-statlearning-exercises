...
# 
# # Initialize vectors to store TPRs and FPRs
...
#   tp = sum(predicted_class == 1 & y_test == 1) # True positives
#   tn = sum(predicted_class == 0 & y_test == 0) # True negatives
#   fp = sum(predicted_class == 1 & y_test == 0) # False positives
#   fn = sum(predicted_class == 0 & y_test == 1) # False negatives
#   
...
# 
# # Plot ROC curve
# plot(fprs, tprs, type = "l", col = "blue", xlab = "False Positive Rate", ylab = "True Positive Rate", main = "ROC Curve")
...
Based on the ROC curve I would recommend a probability threshold of 0.5017094. Since we have a false positive rate of 0 and a true positive of 1, it is safe to say that our model performs exceptionally well at this threshold. However, we still need to be careful because this may be a case of overfitting. To be certain that this exceptional performance is not a product of overfitting, we could test our model on an external dataset to see if it performs similarly. In conclusion, we are indeed able to predict whether a mushroom is poisonous or not by using machine learning (as long as overfitting is not present). 
