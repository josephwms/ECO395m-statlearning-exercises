...
We see that, unsurprisingly, random forest and xgboost do the best out of box.  With some tuning, we are able to identify xgboost as our best possible model.
...
ggplot() +
  ...
  geom_point(data = test_ca, aes(x = test_ca_plot$longitude, y = test_ca_plot$latitude, color = predictedMedianHouseValue), size = 1, alpha = 0.5) +
...
ggplot() +
  geom_polygon(data = california_map, aes(x = long, y = lat, group = group), fill = "lightgray", color = "white") +
  geom_point(data = test_ca, aes(x = test_ca_plot$longitude, y = test_ca_plot$latitude, color = residuals), size = 1, alpha = 0.5) +
