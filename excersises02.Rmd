...
cat(paste('base1 cv RMSE:', base1$results$RMSE %>% round(4)))
```
...
base3 = train(
  form = children ~ hotel + meal + reserved_room_type + poly(adults,2) + poly(average_daily_rate,2) + total_of_special_requests + arrival_date,
  data = hoteldev,
  trControl = trainControl(method = "cv", number = 10),
  method = "glm",
  family = "binomial"
...
cat(paste('base3 RMSE:', base2$results$RMSE %>% round(4)))
